...
function renderOptions(data, result) {
  ...
  const renderSelectedStake = (row, label, key) => { if (!row) { $('selected-stake-card').hidden = true; return; } const guaranteed = Math.min(row.currentProfit, row.opposingProfit); $('selected-stake-card').className = `custom-stake-card selected-stake-card ${key}`; $('selected-stake-card').hidden = false; $('selected-stake-card').innerHTML = `<div><span>Selected balance</span><strong>${dollars(row.hedgeStake)}</strong><em>${label}</em></div><div><span>Position wins</span><b class="${row.currentProfit < 0 ? 'negative' : ''}">${signedMoney(row.currentProfit)}</b></div><div><span>Hedge wins</span><b class="${row.opposingProfit < 0 ? 'negative' : ''}">${signedMoney(row.opposingProfit)}</b></div><div><span>Guaranteed net profit</span><b class="${guaranteed < 0 ? 'negative' : ''}">${signedMoney(guaranteed)}</b></div>`; };
  ...
  $('selected-stake-card').hidden = true;
  ...
  document.querySelectorAll('[data-strategy]').forEach(card => { const key = card.dataset.strategy; if (!result.strategies[key]) return; card.addEventListener('mouseenter', () => setMarkerVisible(key, true)); card.addEventListener('mouseleave', () => setMarkerVisible(key, card.getAttribute('aria-pressed') === 'true')); card.addEventListener('click', () => { const cards = [...document.querySelectorAll('[data-strategy]')], oneSelected = cards.filter(item => item.getAttribute('aria-pressed') === 'true').length === 1 && card.getAttribute('aria-pressed') === 'true'; if (oneSelected) $('selected-stake-card').hidden = true; else renderSelectedStake(result.strategies[key], choices.find(choice => choice.key === key).label, key); cards.forEach(other => { const visible = oneSelected || other === card; other.setAttribute('aria-pressed', String(visible)); other.classList.toggle('is-active', !oneSelected && visible); if (result.strategies[other.dataset.strategy]) setMarkerVisible(other.dataset.strategy, visible); }); }); card.addEventListener('keydown', event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); card.click(); } }); });
}
