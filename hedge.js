...
function renderChart(data, result) {
  ...
  const showCustomStake = event => { const x = hedgeChart.scales.x.getValueForPixel(event.x); const stake = Math.min(data.maxHedgeStake, Math.max(0, x)); const currentProfit = data.payout - data.stake - stake; const opposingProfit = hedgeProfit(data.odds, data.format, stake) - data.stake; const guaranteed = Math.min(currentProfit, opposingProfit); $('custom-stake-card').hidden = false; $('custom-stake-card').innerHTML = `<div><span>Custom hedge stake</span><strong>${dollars(stake)}</strong></div><div><span>Position wins</span><b>${signedMoney(currentProfit)}</b></div><div><span>Hedge wins</span><b>${signedMoney(opposingProfit)}</b></div><div><span>Guaranteed net profit</span><b>${signedMoney(guaranteed)}</b></div>`; };
  hedgeChart = new Chart($('hedge-chart'), { type: 'line', data: { datasets: [
    ...
    marker(result.strategies.current, '#ef765e', 'Position-favored Stake', 'current'), marker(result.strategies.balanced, '#69c99a', 'Neutral Stake', 'balanced'), marker(result.strategies.opposing, '#263c35', 'Hedge-favored Stake', 'opposing')
  ].filter(Boolean) }, options: { responsive: true, maintainAspectRatio: false, interaction: { mode: 'index', intersect: false }, onClick: (event, elements, chart) => { if (event.x >= chart.chartArea.left && event.x <= chart.chartArea.right && event.y >= chart.chartArea.top && event.y <= chart.chartArea.bottom) showCustomStake(event); }, onHover: (event, elements, chart) => { if (event.native && event.native.buttons === 1 && event.x >= chart.chartArea.left && event.x <= chart.chartArea.right && event.y >= chart.chartArea.top && event.y <= chart.chartArea.bottom) showCustomStake(event); }, plugins: { legend: { labels: { color: '#263c35', usePointStyle: false, boxWidth: 30, boxHeight: 3, padding: 16 } }, tooltip: { filter: item => !['Position-favored Stake', 'Neutral Stake', 'Hedge-favored Stake'].includes(item.dataset.label), callbacks: { title: items => `Hedge stake: ${dollars(items[0].parsed.x)}`, label: item => `${item.dataset.label}: ${signedMoney(item.parsed.y)}` } } }, scales: { x: { type: 'linear', min: 0, max: data.maxHedgeStake, title: { display: true, text: 'Hedge stake ($)', color: '#263c35' }, ticks: { color: '#687870', callback: value => dollars(value) }, grid: { color: '#dfe6e0' } }, y: { min: min - padding, max: max + padding, title: { display: true, text: 'Net profit ($)', color: '#263c35' }, ticks: { color: '#687870', callback: value => signedMoney(value) }, grid: { color: '#dfe6e0' } } } } });
}
...
function renderOptions(data, result) {
  ...
  document.querySelectorAll('[data-strategy]').forEach(card => { const key = card.dataset.strategy; if (!result.strategies[key]) return; card.addEventListener('mouseenter', () => setMarkerVisible(key, true)); card.addEventListener('mouseleave', () => setMarkerVisible(key, card.getAttribute('aria-pressed') === 'true')); card.addEventListener('click', () => { document.querySelectorAll('[data-strategy]').forEach(other => { const selected = other === card; other.setAttribute('aria-pressed', String(selected)); other.classList.toggle('is-active', selected); if (result.strategies[other.dataset.strategy]) setMarkerVisible(other.dataset.strategy, selected); }); }); card.addEventListener('keydown', event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); card.click(); } }); });
}
